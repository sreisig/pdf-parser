...
class FileRenamer():
    def __init__(self, input_dir, accept_dir, reject_dir, pdf_type=0):
        '''
        ...
        pdf_type: number corresponding to how pdf formatting version
        '''
    ...
    @staticmethod
    def generate_filename_1(pageObj):
        '''
        Generate filename for pdfs with formatting 1
        '''
    ...
    @staticmethod
    def generate_filename_0(pageObj):
        '''
        Generate filename for pdfs with formatting 0
        '''
        pageText = pageObj.extractText().splitlines()
        for i in range(len(pageText)):
            if pageText[i] == 'Service Level ':
                acc_name = pageText[i-1]
                acc_name = re.sub("^[0-9]+%?", '', acc_name)
                acc_name = re.sub("^\$-?[0-9,]*", '', acc_name)
                acc_name = acc_name.replace(' ', '_') + '.pdf'
                acc_name = re.sub("_\.pdf", '.pdf', acc_name)
                return acc_name
        return None
    ...
    def process_dir(self):
        ...
        for filename in os.listdir(self.input_dir):
            if filename.endswith('.pdf'):
                ...
                if self.pdf_type == 0:
                    base_possible_fname = self.generate_filename_0(pageObj)
                elif self.pdf_type == 1:
                    base_possible_fname = self.generate_filename_1(pageObj)
